...
Conecta a tu instancia n8n vía API REST, muestra todos tus workflows en una interfaz interactiva de terminal y exporta los que selecciones como archivos `.json` limpios — idénticos a los que produce el botón "Download" de n8n, sin metadatos de la instancia origen.
...
Auth type — 1 for Basic / 2 for API Token: 2
API Token: ••••••••••••••••
...
El CLI soporta dos métodos:

**API Token** (recomendado) — Settings → API → Create an API key en tu instancia n8n.

**Basic Auth** — usuario y contraseña de tu instancia. Disponible en algunas configuraciones self-hosted.
